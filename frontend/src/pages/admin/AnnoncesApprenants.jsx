...
export default function AnnoncesApprenants() {
  ...
  const fetchAnnonces = async () => {
    try {
      const res = await API.get('/admin/annonces/apprenants');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await API.post('/admin/annonces/apprenants', { titre, contenu });
      toast.success('Annonce envoyée aux apprenants');
      setTitre('');
      setContenu('');
      fetchAnnonces();
    } catch (e) {
  ...
  return (
    <div className="p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Annonces Apprenants</h1>
          <p className="text-gray-600">Rédigez et envoyez une annonce à tous les apprenants.</p>
        </div>
